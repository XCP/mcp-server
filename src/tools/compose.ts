...
export function registerComposeTools(server: McpServer, client: ApiClient) {
  ...
  server.tool(
    ...
    'Compose a transaction to send Counterparty tokens to an address. Returns unsigned transaction hex and PSBT.',
    {
      ...
      quantity: z.number().describe('Amount to send (in satoshis for divisible assets, or whole units for indivisible)'),
  ...
  server.tool(
    ...
    async ({ address, destinations, ...params }) => {
  ...
  server.tool(
    ...
    'Compose a DEX order to trade Counterparty assets. Creates a limit order on the decentralized exchange.',
    {
      ...
      give_quantity: z.number().describe('Amount of asset to give'),
      get_asset: z.string().describe('Asset to receive/buy'),
      get_quantity: z.number().describe('Amount of asset to receive'),
  ...
  server.tool(
    ...
    'Compose a transaction to issue (create) a new Counterparty asset, or update an existing one.',
    {
      ...
      quantity: z.number().describe('Amount to issue'),
  ...
  server.tool(
    ...
    'Compose a transaction to create, open, or close a dispenser. Dispensers automatically sell tokens for BTC.',
    {
      ...
      give_quantity: z.number().describe('Amount of asset to give per dispense'),
      escrow_quantity: z.number().describe('Total amount of asset to load into dispenser'),
  ...
  server.tool(
    ...
    'Compose a transaction to buy from a dispenser by sending BTC to it',
    {
      ...
      quantity: z.number().describe('Amount of BTC to send (in satoshis)'),
      ...feeOptions,
      ...utxoOptions,
    },
    async ({ address, ...params }) => {
  ...
  server.tool(
    ...
    'Compose a transaction to distribute dividends to all holders of an asset',
    {
      ...
      quantity_per_unit: z.number().describe('Amount of dividend asset per unit of held asset'),
      ...feeOptions,
      ...utxoOptions,
    },
    async ({ address, ...params }) => {
  ...
  server.tool(
    ...
    'Compose a transaction to permanently destroy (burn) Counterparty tokens',
    {
      ...
      quantity: z.number().describe('Amount to destroy'),
  ...
  server.tool(
    ...
    'Compose a transaction to create a fair minting launch for a new asset',
    {
      ...
      max_mint_per_tx: z.number().optional().describe('Maximum amount that can be minted per transaction'),
      hard_cap: z.number().optional().describe('Maximum total supply'),
      ...
      soft_cap: z.number().optional().describe('Soft cap for the minting'),
      ...
      premint_quantity: z.number().optional().describe('Amount to premint to the issuer'),
  ...
  server.tool(
    ...
    'Compose a transaction to mint tokens from an active fair minter',
    {
      ...
      quantity: z.number().optional().describe('Amount to mint (if not specified, mints max allowed)'),
      ...feeOptions,
      ...utxoOptions,
    },
    async ({ address, ...params }) => {
  ...
  server.tool(
    ...
    'Compose a transaction to attach Counterparty tokens to a specific UTXO',
    {
      ...
      quantity: z.number().describe('Amount to attach'),
